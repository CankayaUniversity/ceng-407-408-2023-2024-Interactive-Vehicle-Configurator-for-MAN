  ...
  <v-container>
    ...
    <v-row>
      <v-col>
        ...
        <v-card class="custom-part">
          <v-card-title>
            Custom Part
            <v-dialog v-model="dialogAddProduct" max-width="50%">
              <template v-slot:activator="{ props: activatorProps }">
                <v-btn
                  v-bind="activatorProps"
                  size="small"
              ...
              <v-card>
                ...
                <v-card-actions>
                  ...
                  <v-btn
                    ...
                    >Close</v-btn
                  >
                  <v-btn color="blue darken-1" text @click="submitAddProduct"
                    >Add</v-btn
                  >
                </v-card-actions>
              </v-card>
    ...
    <v-dialog v-model="dialogAddOption" max-width="50%">
      ...
      <v-card>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="blue darken-1" text @click="dialogAddOption = false"
            >Close</v-btn
          >
...
export default {
  data() {
    return {
      ...
      dialogAddProduct: false, // Add Product dialogu için
      addOptionOptions: "", // Eklenecek options, virgülle ayrılmış string
    };
  },
  mounted() {
  ...
  methods: {
    ...
    async deleteProduct(productId) {
      try {
        ...
        if (response.status === 200) {
          ...
          this.fetchProducts(); // Update product list
        } else {
    ...
    async submitAddOption() {
      try {
        const productName = this.addOptionName;
        const productOptions = this.addOptionOptions
          .split(",")
        ...
        if (!productName || !mainGroupID) {
          alert("ProductName and MainGroupID are required");
          return;
        }
        ...
        const response = await axios.post(
          "http://localhost:3000/datauploadrequests",
          {
            UserID: 1, // Admin ID
            TableName: "Products",
            RequestDetails: `Action: Add Option, ${JSON.stringify(
              requestDetails
            )}`,
            RequestStatus: false,
            RequestDate: new Date(),
