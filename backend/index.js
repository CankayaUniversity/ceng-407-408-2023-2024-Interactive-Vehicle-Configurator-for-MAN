  ...
  try {
    ...
    try {
      parsedDetails = JSON.parse(
        RequestDetails.split("Action: Add Option, ")[1]
      );
    } catch (error) {
      console.log("JSON parse error:", error);
      return res.status(400).send("Invalid request details format");
    }
    ...
    if (ActionType === "Add Option") {
      const { ProductName, Options, MainGroupID, GattungID } = parsedDetails;
      console.log("Add Option Details:", {
        ProductName,
        Options,
        MainGroupID,
        GattungID,
      ...
      query = `INSERT INTO Products (Name, Options, MainGroupID, GattungID, InputType, InputPlaceholder) VALUES (@ProductName, @Options, @MainGroupID, @GattungID, @InputType, @InputPlaceholder)`;
      inputs = [
        ...
        { name: "GattungID", type: sql.Int, value: GattungID || null },
        { name: "InputType", type: sql.NVarChar, value: null }, // Varsayılan olarak null
        { name: "InputPlaceholder", type: sql.NVarChar, value: null }, // Varsayılan olarak null
      ];
    }
    ...
    if (query) {
      ...
      await pool
        ...
        .input("RequestID", sql.Int, id)
        .query(
          "UPDATE DataUploadRequests SET RequestStatus = 1 WHERE RequestID = @RequestID"
        );
